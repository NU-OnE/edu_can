...
public class UserDao {
	...
	public ResposeResult store(User user) {
		...
		try {
			...
			PreparedStatement stmt = con
					.prepareStatement("insert into users (name, role, email, password, active) values (?,?,?,?,?)");
	...
	public ResposeResult getAllUsers() {
		...
		try {
			...
			while (result.next()) {
				User user = new User(result.getInt(1), result.getString(2), result.getString(3), "pass",
